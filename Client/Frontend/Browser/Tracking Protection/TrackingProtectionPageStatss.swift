...
struct TPPageStatss {
    ...
    func create(byAddingListItem listItem: BlocklistName) -> TPPageStatss {
        switch listItem {
        case .advertising: return TPPageStatss(adCount: adCount + 1, analyticCount: analyticCount, contentCount: contentCount, socialCount: socialCount)
        case .analytics: return TPPageStatss(adCount: adCount, analyticCount: analyticCount + 1, contentCount: contentCount, socialCount: socialCount)
        case .content: return TPPageStatss(adCount: adCount, analyticCount: analyticCount, contentCount: contentCount + 1, socialCount: socialCount)
        case .social: return TPPageStatss(adCount: adCount, analyticCount: analyticCount, contentCount: contentCount, socialCount: socialCount + 1)
        }
    }
}

class TPStatsBlocklistCheckers {
    static let shared = TPStatsBlocklistCheckers()

    private var blockLists: TPStatsBlocklistss?
...
func wildcardContentBlockerDomainToRegexs(domain: String) -> NSRegularExpression? {
...
private class TPStatsBlocklistss {
    class Rule {
        ...
        for blockList in BlocklistName.all {
            ...
            for rule in list {
                ...
                let domainExceptionsRegex = (trigger["unless-domain"] as? [String])?.compactMap { domain in
                    return wildcardContentBlockerDomainToRegexs(domain: domain)
                }
