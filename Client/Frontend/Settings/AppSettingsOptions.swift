...
class ContentBlockerSetting: Setting {
    ...
    override func onClick(_ navigationController: UINavigationController?) {
        if let tab = self.tabManager.selectedTab {
            let etpViewModel = TrackingProtectionVM(tab: tab, profile: profile)
            let trackingProtectionVC = TrackingProtectionViewController(state: .settings, prefs: profile.prefs, viewModel: etpViewModel)
